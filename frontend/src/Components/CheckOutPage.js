...
function CheckOutPage() {
    ...
    useEffect(() => {
        ...
        console.log("Loaded cart from localStorage:", storedCart); // ✅ Check this
        setCart(storedCart);
        const quantity = storedCart.reduce((acc, item) => acc + item.quantity, 0);
        localStorage.setItem("cartQuantity", quantity);
        console.log("weerrr", quantity)
    }, []);
    ...
    // localStorage.setItem("cartQuantity", totalquantity); // 💾 Save to localStorage


    const navigate = useNavigate()
    const goToAddressPage = () => {
        navigate('/AddressDetailsPage', { state: { MRPTotal, total, discount } });
    };
    ...
    const handlePayment = async () => {
        ...
        const { key, order } = await res.json(); // key_id & order object
        console.log("key", key)

        const options = {
            key: key, // Razorpay key_id (public)
            amount: order.amount,
            currency: order.currency,
            order_id: order.id,
            name: "Test Store",
            description: "Payment without webhook",
            handler: async function (response) {
                // Razorpay sends payment data after success
                        ...
                        <div className='Cart-Part '>
                            <div style={{ display: "flex" }}>
                                <div style={{ width: "75%", textAlign: "left" }}>
                                    <h5>Shopping Cart({totalquantity})</h5>
                                </div>
                                <div style={{ width: "25%" }}>
                                    <div style={{ marginBottom: "10px" }} onClick={goToAddressPage}>
                                        <button style={{
                                            fontWeight: " bold",
                                            cursor: "pointer",
                                            padding: "3.5px 10px",
